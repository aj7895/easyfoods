...
const Mailchimp = ({ status, message, onValidated }) => {
  ...
  function submit() {
    ...
    {
      status === "error" && alert.warn("email already subscribed with us");
    }
    {
      status === "success" &&
        alert.success("Thank you for subscribing with us");
    }
  }
  ...
  return (
    <div className="flex items-center justify-between border border-gray-800 mt-4">
      ...
      />
      <button onClick={submit} className="mr-4 cursor-pointer relative z-40">
        <svg
